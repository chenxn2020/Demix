...
class Model(nn.Module):
    ...
    def get_emb(self, query, mode):
        if query.shape == torch.Size([]):
            query = query.view(1)
        if mode == 'entity':
            return self.ent_emb(query) #shape:[num, dim]
        else:
            return self.rel_emb(query) #shape:[1, dim]


    # def get_tri_emb(self, triples, mode="single"):
    #     #---------拼接得到三元组的emb
    #     if mode == "single":
    #         head_emb = self.ent_emb(triples[:, 0]).unsqueeze(1)  # [bs, 1, dim]
    #         relation_emb = self.rel_emb(triples[:, 1]).unsqueeze(1)  # [bs, 1, dim]
