...
class KBAT(nn.Module):
    ...
    def init_GAT_emb(self):
        ...
        #self.ent_emb_out = nn.Parameter(torch.randn(self.args.num_ent,self.args.emb_dim))
        #self.rel_emb_out = nn.Parameter(torch.randn(self.args.num_rel,self.args.emb_dim))

        self.ent_emb_out, self.rel_emb_out = init_embeddings(os.path.join(self.args.data_path, 'entity2vec.txt'), os.path.join(self.args.data_path, 'relation2vec.txt'))

        self.ent_emb_out = nn.Parameter(torch.FloatTensor(self.ent_emb_out))
        self.rel_emb_out = nn.Parameter(torch.FloatTensor(self.rel_emb_out))
    ...
    def forward_GAT(self, triples, adj_matrix, n_hop):
        ...
        mask = torch.zeros(self.ent_emb_out.shape[0]).cuda() #todo:type_as()
